...
def polygonFromBody(zLevel: float, mesh: 'pv.PolyData', cutoff: float = APPROX_ZERO) -> Polygon:
    """
    ...
    -------
    Polygon
        largest polygon resulting from z slice
    """
    polygon: Polygon = None
    if mesh.bounds[4] > zLevel or mesh.bounds[5] < zLevel:
        return None
    for p in polygonsFromMesh(zLevel, mesh, cutoff=cutoff):
